    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-4">
      ...
      <section class="mt-6" aria-labelledby="steps-heading">
        ...
        <div class="space-y-1">
          <label v-for="step in steps" :key="step.id" class="flex min-h-10 items-center gap-3 rounded px-2 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800">
            <input
              ...
              :aria-label="`${t('completeStep')} ${step.title}`"
            ...
            <input
              ...
              type="text"
              :aria-label="`${t('editStep')} ${step.title}`"
              autofocus
              @blur="saveStepTitle"
        ...
        <form class="mt-2 flex items-center gap-2 border-b border-slate-200 px-2 py-2 dark:border-slate-700" @submit.prevent="addStep">
          ...
          <label class="sr-only" for="new-step-title">{{ t('addStep') }}</label>
          <input id="new-step-title" v-model="stepTitle" class="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-slate-500" type="text" :placeholder="t('addStep')" />
        </form>
      </section>
